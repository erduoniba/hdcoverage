...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData jx_imageFormatForImageData:data] == SDImageFormatWebP);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable SDImageCoderOptions *)options {
    ...
    CGColorSpaceRef colorSpace = [self jx_createColorSpaceWithDemuxer:demuxer];
    ...
    if (!hasAnimation || decodeFirstFrame) {
        ...
        CGImageRef imageRef = [self jx_createWebpImageWithData:iter.fragment colorSpace:colorSpace scaledSize:scaledSize];
...
#endif
        firstFrameImage.jx_imageFormat = SDImageFormatWebP;
    ...
    do {
        @autoreleasepool {
            CGImageRef imageRef = [self jx_drawnWebpImageWithCanvas:canvas iterator:iter colorSpace:colorSpace scaledSize:scaledSize];
            if (!imageRef) {
                continue;
            }
            ...
            NSTimeInterval duration = [self jx_frameDurationWithIterator:iter];
    ...
    animatedImage.jx_imageLoopCount = loopCount;
    animatedImage.jx_imageFormat = SDImageFormatWebP;
...
- (UIImage *)incrementalDecodedImageWithOptions:(SDImageCoderOptions *)options {
    ...
    if (_demux) {
        ...
        SD_UNLOCK(_lock);
        image.jx_imageFormat = SDImageFormatWebP;
        image.jx_isDecoded = YES;
        return image;
    }
...
#endif
        image.jx_isDecoded = YES;  // Already drawn on bitmap context above
        image.jx_imageFormat = SDImageFormatWebP;
        CGImageRelease(newImageRef);
        CGContextRelease(canvas);
    }
...
- (void)jx_blendWebpImageWithCanvas:(CGContextRef)canvas iterator:(WebPIterator)iter colorSpace:(nonnull CGColorSpaceRef)colorSpaceRef {
    ...
    if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
        ...
        CGImageRef imageRef = [self jx_createWebpImageWithData:iter.fragment colorSpace:colorSpaceRef scaledSize:CGSizeZero];
        if (!imageRef) {
            return;
        }
...
- (nullable CGImageRef)jx_drawnWebpImageWithCanvas:(CGContextRef)canvas
                                          iterator:(WebPIterator)iter
                                        colorSpace:(nonnull CGColorSpaceRef)colorSpaceRef
                                        scaledSize:(CGSize)scaledSize CF_RETURNS_RETAINED {
    CGImageRef imageRef = [self jx_createWebpImageWithData:iter.fragment colorSpace:colorSpaceRef scaledSize:CGSizeZero];
    if (!imageRef) {
        return nil;
    }
...
- (nullable CGImageRef)jx_createWebpImageWithData:(WebPData)webpData colorSpace:(nonnull CGColorSpaceRef)colorSpaceRef scaledSize:(CGSize)scaledSize CF_RETURNS_RETAINED {
...
- (NSTimeInterval)jx_frameDurationWithIterator:(WebPIterator)iter {
...
- (nonnull CGColorSpaceRef)jx_createColorSpaceWithDemuxer:(nonnull WebPDemuxer *)demuxer CF_RETURNS_RETAINED {
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format options:(nullable SDImageCoderOptions *)options {
    ...
    if (encodeFirstFrame || frames.count == 0) {
        ...
        data = [self jx_encodedWebpDataWithImage:image.CGImage quality:compressionQuality maxPixelSize:maxPixelSize maxFileSize:maxFileSize];
    } else {
        // for animated webp image
        ...
        for (size_t i = 0; i < frames.count; i++) {
            ...
            NSData *webpData = [self jx_encodedWebpDataWithImage:currentFrame.image.CGImage quality:compressionQuality maxPixelSize:maxPixelSize maxFileSize:maxFileSize];
        ...
        int loopCount = (int)image.jx_imageLoopCount;
        WebPMuxAnimParams params = {.bgcolor = 0, .loop_count = loopCount};
...
- (nullable NSData *)jx_encodedWebpDataWithImage:(nullable CGImageRef)imageRef quality:(double)quality maxPixelSize:(CGSize)maxPixelSize maxFileSize:(NSUInteger)maxFileSize {
    NSData *webpData;
    if (!imageRef) {
        return nil;
    }
    ...
    if (byteOrderNormal && ((alphaInfo == kCGImageAlphaNone) || (alphaInfo == kCGImageAlphaLast))) {
        ...
        vImage_CGImageFormat srcFormat = {.bitsPerComponent = (uint32_t)CGImageGetBitsPerComponent(imageRef),
        ...
        vImage_Buffer src = {.data = (uint8_t *)CFDataGetBytePtr(dataRef), .width = width, .height = height, .rowBytes = bytesPerRow};
        vImage_Buffer dest;
    ...
    if (!WebPConfigPreset(&config, WEBP_PRESET_DEFAULT, qualityFactor) || !WebPPictureInit(&picture)) {
...
- (BOOL)scanAndCheckFramesValidWithDemuxer:(WebPDemuxer *)demuxer {
    ...
    do {
        ...
        frame.duration = [self jx_frameDurationWithIterator:iter];
...
- (UIImage *)safeStaticImageFrame {
    ...
    if (!_colorSpace) {
        _colorSpace = [self jx_createColorSpaceWithDemuxer:_demux];
    }
    // Check whether we need to use thumbnail
    CGImageRef imageRef;
    if (_hasAnimation) {
        ...
        imageRef = [self jx_drawnWebpImageWithCanvas:_canvas iterator:iter colorSpace:_colorSpace scaledSize:scaledSize];
    } else {
        CGSize scaledSize = SDCalculateThumbnailSize(CGSizeMake(iter.width, iter.height), _preserveAspectRatio, _thumbnailSize);
        imageRef = [self jx_createWebpImageWithData:iter.fragment colorSpace:_colorSpace scaledSize:scaledSize];
    }
    if (!imageRef) {
        return nil;
    }
...
- (UIImage *)safeAnimatedImageFrameAtIndex:(NSUInteger)index {
    ...
    // Because Animated WebP supports dispose method, which means frames can based on previous canvas context. However, if we clear canvas and loop from the 0 index until the request index, it's harm
    // for performance. But when one frame's dispose method is `WEBP_MUX_DISPOSE_BACKGROUND`, the canvas is cleared after the frame decoded. And subsequent frames are not effected by that frame. So,
    // we calculate each frame's `blendFromIndex`. Then directly draw canvas from that index, instead of always from 0 index.
    ...
    if (_currentBlendIndex != NSNotFound && _currentBlendIndex + 1 == index) {
        ...
        if (endIndex > startIndex) {
            do {
                @autoreleasepool {
                    [self jx_blendWebpImageWithCanvas:_canvas iterator:iter colorSpace:_colorSpace];
                }
    ...
    // Check whether we need to use thumbnail
    CGSize scaledSize = SDCalculateThumbnailSize(CGSizeMake(_canvasWidth, _canvasHeight), _preserveAspectRatio, _thumbnailSize);
    CGImageRef imageRef = [self jx_drawnWebpImageWithCanvas:_canvas iterator:iter colorSpace:_colorSpace scaledSize:scaledSize];
    if (!imageRef) {
        return nil;
    }
#if SD_UIKIT || SD_WATCH
