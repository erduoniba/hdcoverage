...
- (void)storeImage:(nullable UIImage *)image
    ...
    if (image && toMemory && self.config.shouldCacheImagesInMemory) {
        NSUInteger cost = image.jx_memoryCost;
    ...
    dispatch_async(self.ioQueue, ^{
      @autoreleasepool {
          ...
          if (!data && image) {
              ...
              SDImageFormat format = image.jx_imageFormat;
...
- (void)_archivedDataWithImage:(UIImage *)image forKey:(NSString *)key {
    ...
    // Check extended data
    id extendedObject = image.jx_extendedObject;
...
- (nullable UIImage *)imageFromDiskCacheForKey:(nullable NSString *)key options:(SDImageCacheOptions)options context:(nullable SDWebImageContext *)context {
    ...
    if (diskImage && self.config.shouldCacheImagesInMemory && shouldCacheToMomery) {
        NSUInteger cost = diskImage.jx_memoryCost;
...
#pragma clang diagnostic pop
    ...
    image.jx_extendedObject = extendedObject;
}
...
- (nullable NSOperation *)queryCacheOperationForKey:(nullable NSString *)key
    ...
    if (image) {
        if (options & SDImageCacheDecodeFirstFrameOnly) {
            ...
            if (image.jx_isAnimated || ([animatedImageClass isSubclassOfClass:[UIImage class]] && [animatedImageClass conformsToProtocol:@protocol(SDAnimatedImage)])) {
#if SD_MAC
    ...
    BOOL shouldQueryDiskSync = ((image && options & SDImageCacheQueryMemoryDataSync) || (!image && options & SDImageCacheQueryDiskDataSync));
...
@implementation PGSDImageCache (SDImageCache)
