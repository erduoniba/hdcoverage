...
+ (BOOL)canDecodeFromFormat:(SDImageFormat)format {
    ...
    CFStringRef imageUTType = [NSData jx_UTTypeFromImageFormat:format];
...
+ (UIImage *)createFrameAtIndex:(NSUInteger)index
    ...
    // Check vector format
    BOOL isVector = NO;
    if ([NSData jx_imageFormatFromUTType:uttype] == SDImageFormatPDF) {
        isVector = YES;
    }
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData jx_imageFormatForImageData:data] == self.class.imageFormat);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable SDImageCoderOptions *)options {
    ...
    animatedImage.jx_imageFormat = self.class.imageFormat;
    CFRelease(source);
...
- (UIImage *)incrementalDecodedImageWithOptions:(SDImageCoderOptions *)options {
    ...
    if (_width + _height > 0) {
        ...
        if (image) {
            image.jx_imageFormat = self.class.imageFormat;
        }
    }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(SDImageFormat)format options:(nullable SDImageCoderOptions *)options {
    ...
    if (encodeFirstFrame || frames.count == 0) {
        ...
        // for animated images
        NSUInteger loopCount = image.jx_imageLoopCount;
        NSDictionary *containerProperties = @{self.class.dictionaryProperty : @{self.class.loopCountProperty : @(loopCount)}};
...
- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    ...
    // Animated Image should not use the CGContext solution to force decode. Prefers to use Image/IO built in method, which is safer and memory friendly, see
    // https://github.com/SDWebImage/SDWebImage/issues/2961
    NSDictionary *options = @{
        (__bridge NSString *)kCGImageSourceShouldCacheImmediately : @(YES),
        (__bridge NSString *)kCGImageSourceShouldCache : @(YES)  // Always cache to reduce CPU usage
    };
