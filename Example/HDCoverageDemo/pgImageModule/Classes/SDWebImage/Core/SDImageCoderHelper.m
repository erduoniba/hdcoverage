...
static inline size_t SDByteAlign(size_t size, size_t alignment) { return ((size + (alignment - 1)) / alignment) * alignment; }
...
+ (UIImage *)animatedImageWithFrames:(NSArray<SDImageFrame *> *)frames {
    ...
    CFStringRef imageUTType = [NSData jx_UTTypeFromImageFormat:SDImageFormatGIF];
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage orientation:(CGImagePropertyOrientation)orientation {
    ...
    // Though you can use any supported bitmapInfo (see:
    // https://developer.apple.com/library/content/documentation/GraphicsImaging/Conceptual/drawingwithquartz2d/dq_context/dq_context.html#//apple_ref/doc/uid/TP30001066-CH203-BCIBHHBB ) and let Core
    // Graphics reorder it when you call `CGContextDrawImage` But since our build-in coders use this bitmapInfo, this can have a little performance benefit
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    @autoreleasepool {
        ...
        destContext = CGBitmapContextCreate(NULL, destResolution.width, destResolution.height, kBitsPerComponent, 0, colorspaceRef, bitmapInfo);
...
+ (BOOL)shouldDecodeImage:(nullable UIImage *)image {
    ...
    // Avoid extra decode
    if (image.jx_isDecoded) {
        return NO;
    }
    // do not decode animated images
    if (image.jx_isAnimated) {
        return NO;
    }
    // do not decode vector images
    if (image.jx_isVector) {
        return NO;
    }
