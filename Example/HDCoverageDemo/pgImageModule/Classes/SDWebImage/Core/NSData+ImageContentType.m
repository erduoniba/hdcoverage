...
+ (SDImageFormat)jx_imageFormatForImageData:(nullable NSData *)data {
    if (!data) {
    ...
    switch (c) {
        ...
        case 0x3C: {
            ...
            if ([data rangeOfData:[kSVGTagEnd dataUsingEncoding:NSUTF8StringEncoding] options:NSDataSearchBackwards range:NSMakeRange(data.length - MIN(100, data.length), MIN(100, data.length))]
                    .location != NSNotFound) {
...
+ (nonnull CFStringRef)jx_UTTypeFromImageFormat:(SDImageFormat)format {
    CFStringRef UTType;
    switch (format) {
...
+ (SDImageFormat)jx_imageFormatFromUTType:(CFStringRef)uttype {
    if (!uttype) {
        return SDImageFormatUndefined;
    }
