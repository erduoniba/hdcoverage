...
- (NSUInteger)jx_imageLoopCount {
    NSUInteger imageLoopCount = 0;
    NSNumber *value = objc_getAssociatedObject(self, @selector(jx_imageLoopCount));
...
- (void)setJx_imageLoopCount:(NSUInteger)jx_imageLoopCount {
    NSNumber *value = @(jx_imageLoopCount);
    objc_setAssociatedObject(self, @selector(jx_imageLoopCount), value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setJx_imageLoopCount:(NSUInteger)jx_imageLoopCount {
    ...
    if (bitmapImageRep) {
        [bitmapImageRep setProperty:NSImageLoopCount withValue:@(jx_imageLoopCount)];
    }
}
...
- (SDImageFormat)jx_imageFormat {
    SDImageFormat imageFormat = SDImageFormatUndefined;
    NSNumber *value = objc_getAssociatedObject(self, @selector(jx_imageFormat));
    if ([value isKindOfClass:[NSNumber class]]) {
    ...
    if (@available(iOS 9.0, tvOS 9.0, macOS 10.11, watchOS 2.0, *)) {
        ...
        imageFormat = [NSData jx_imageFormatFromUTType:uttype];
    }
    return imageFormat;
}

- (void)setJx_imageFormat:(SDImageFormat)jx_imageFormat {
    objc_setAssociatedObject(self, @selector(jx_imageFormat), @(jx_imageFormat), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)setJx_isIncremental:(BOOL)jx_isIncremental {
    objc_setAssociatedObject(self, @selector(jx_isIncremental), @(jx_isIncremental), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (BOOL)jx_isIncremental {
    NSNumber *value = objc_getAssociatedObject(self, @selector(jx_isIncremental));
