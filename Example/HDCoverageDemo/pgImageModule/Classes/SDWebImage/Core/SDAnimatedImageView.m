...
- (void)setImage:(UIImage *)image {
    ...
    // We need call super method to keep function. This will impliedly call `setNeedsDisplay`. But we have no way to avoid this when using animated image. So we call `setNeedsDisplay` again at the
    // end.
    super.image = image;
...
- (id<SDAnimatedImageCoder, SDProgressiveImageCoder>)progressiveAnimatedCoderForImage:(UIImage *)image {
    if ([image.class conformsToProtocol:@protocol(SDAnimatedImage)] && image.jx_isIncremental && [image respondsToSelector:@selector(animatedCoder)]) {
...
// Why using this design may because of properties like `imageAlignment` and `imageScaling`, which it's not available for UIImageView.contentMode (it's impossible to align left and keep aspect ratio
// at the same time)
- (NSView *)imageView {
