 ...
 */
@property (nonatomic, assign) NSUInteger jx_imageLoopCount;
 ...
 */
@property (nonatomic, assign, readonly) BOOL jx_isAnimated;
 ...
 */
@property (nonatomic, assign, readonly) BOOL jx_isVector;

/**
 * The image format represent the original compressed image data format.
 * If you don't manually specify a format, this information is retrieve from CGImage using `CGImageGetUTType`, which may return nil for non-CG based image. At this time it will return
 * `SDImageFormatUndefined` as default value.
